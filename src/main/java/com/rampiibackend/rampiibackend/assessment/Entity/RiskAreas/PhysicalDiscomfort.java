package com.rampiibackend.rampiibackend.assessment.Entity.RiskAreas;

import com.fasterxml.jackson.annotation.JsonIdentityInfo;
import com.fasterxml.jackson.annotation.JsonIgnore;
import com.fasterxml.jackson.annotation.ObjectIdGenerators;
import lombok.NoArgsConstructor;

import javax.persistence.*;
import javax.validation.constraints.*;


@NoArgsConstructor
@Entity
@Table(name ="physicaldiscomfort")
@JsonIdentityInfo(scope = PhysicalDiscomfort.class,generator= ObjectIdGenerators.PropertyGenerator.class, property="id")
public class PhysicalDiscomfort {

    @JsonIgnore
    private final int SIZE = 65;

    @JsonIgnore
    private final int LENGTH = 5;

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "id")
    private Long id;

    @Column(name = "q71", nullable = true,length = LENGTH)
    @Size(max = LENGTH, message = "It's Max Size")
    private String q71;

    @Column(name = "q71_Comm", nullable = true, length =  SIZE)
    @Size(max = SIZE, message = "Comment Max Length Is 65")
    private String q71_Comm;

    @Column(name = "q72a", nullable = true, length =  SIZE)
    @Size(max = SIZE, message = "Text Max Length Is 65")
    private String q72a;

    @Column(name = "q72a_Comm", nullable = true, length =  SIZE)
    @Size(max = SIZE, message = "Comment Max Length Is 65")
    private String q72a_Comm;

    @Column(name = "q72b", nullable = true, length =  SIZE)
    @Size(max = SIZE, message = "Text Max Length Is 65")
    private String q72b;

    @Column(name = "q72b_Comm", nullable = true, length =  SIZE)
    @Size(max = SIZE, message = "Comment Max Length Is 65")
    private String q72b_Comm;

    @Column(name = "q72c", nullable = true, length =  SIZE)
    @Size(max = SIZE, message = "Text Max Length Is 65")
    private String q72c;

    @Column(name = "q72c_Comm", nullable = true, length =  SIZE)
    @Size(max = SIZE, message = "Comment Max Length Is 65")
    private String q72c_Comm;

    @Column(name = "q72d", nullable = true, length =  SIZE)
    @Size(max = SIZE, message = "Text Max Length Is 65")
    private String q72d;

    @Column(name = "q72d_Comm", nullable = true, length =  SIZE)
    @Size(max = SIZE, message = "Comment Max Length Is 65")
    private String q72d_Comm;

    @Column(name = "q72e", nullable = true, length =  SIZE)
    @Size(max = SIZE, message = "Text Max Length Is 65")
    private String q72e;

    @Column(name = "q72e_Comm", nullable = true, length =  SIZE)
    @Size(max = SIZE, message = "Comment Max Length Is 65")
    private String q72e_Comm;

    @OneToOne(fetch = FetchType.LAZY, cascade = CascadeType.ALL, mappedBy = "physicalDiscomfort")
    @JsonIgnore
    private CheckList checkList;

    public PhysicalDiscomfort(String q71, String q71_Comm,
                              String q72a, String q72a_Comm,
                              String q72b, String q72b_Comm,
                              String q72c, String q72c_Comm,
                              String q72d, String q72d_Comm,
                              String q72e, String q72e_Comm) {
        this.q71 = q71;
        this.q71_Comm = q71_Comm;
        this.q72a = q72a;
        this.q72a_Comm = q72a_Comm;
        this.q72b = q72b;
        this.q72b_Comm = q72b_Comm;
        this.q72c = q72c;
        this.q72c_Comm = q72c_Comm;
        this.q72d = q72d;
        this.q72d_Comm = q72d_Comm;
        this.q72e = q72e;
        this.q72e_Comm = q72e_Comm;
    }

    public Long getId() {
        return id;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public String getQ71() {
        return q71;
    }

    public void setQ71(String q71) {
        this.q71 = q71;
    }

    public String getQ71_Comm() {
        return q71_Comm;
    }

    public void setQ71_Comm(String q71_Comm) {
        this.q71_Comm = q71_Comm;
    }

    public String getQ72a() {
        return q72a;
    }

    public void setQ72a(String q72a) {
        this.q72a = q72a;
    }

    public String getQ72a_Comm() {
        return q72a_Comm;
    }

    public void setQ72a_Comm(String q72a_Comm) {
        this.q72a_Comm = q72a_Comm;
    }

    public String getQ72b() {
        return q72b;
    }

    public void setQ72b(String q72b) {
        this.q72b = q72b;
    }

    public String getQ72b_Comm() {
        return q72b_Comm;
    }

    public void setQ72b_Comm(String q72b_Comm) {
        this.q72b_Comm = q72b_Comm;
    }

    public String getQ72c() {
        return q72c;
    }

    public void setQ72c(String q72c) {
        this.q72c = q72c;
    }

    public String getQ72c_Comm() {
        return q72c_Comm;
    }

    public void setQ72c_Comm(String q72c_Comm) {
        this.q72c_Comm = q72c_Comm;
    }

    public String getQ72d() {
        return q72d;
    }

    public void setQ72d(String q72d) {
        this.q72d = q72d;
    }

    public String getQ72d_Comm() {
        return q72d_Comm;
    }

    public void setQ72d_Comm(String q71d_Comm) {
        this.q72d_Comm = q71d_Comm;
    }

    public String getQ72e() {
        return q72e;
    }

    public void setQ72e(String q72e) {
        this.q72e = q72e;
    }

    public String getQ72e_Comm() {
        return q72e_Comm;
    }

    public void setQ72e_Comm(String q72e_Comm) {
        this.q72e_Comm = q72e_Comm;
    }

    public CheckList getCheckList() {
        return checkList;
    }

    public void setCheckList(CheckList checkList) {
        this.checkList = checkList;
    }

    @Override
    public String toString() {
        return "PhysicalDiscomfort Entity{" +
                "id=" + id +
                ", q71=" + q71 +
                ", q71_Comm='" + q71_Comm + '\'' +
                ", q72a=" + q72a +
                ", q72a_Comm='" + q72a_Comm + '\'' +
                ", q72b=" + q72b +
                ", q72b_Comm='" + q72b_Comm + '\'' +
                ", q72c=" + q72c +
                ", q72c_Comm='" + q72c_Comm + '\'' +
                ", q72d=" + q72d +
                ", q72d_Comm='" + q72d_Comm + '\'' +
                ", q72e=" + q72e +
                ", q72e_Comm='" + q72e_Comm + '\'' +
                '}';
    }
}
